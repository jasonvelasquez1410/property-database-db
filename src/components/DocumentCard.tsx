...
export const DocumentCard = ({ document }: DocumentCardProps) => {
    const handleClick = (e: React.MouseEvent) => {
        if (document.documentUrl === '#' || !document.documentUrl) {
            e.preventDefault();
            alert("This is a demo document. In a real app, this would open the file.");
        }
    };

    return (
        <a href={document.documentUrl} onClick={handleClick} target="_blank" rel="noopener noreferrer" className="border rounded-lg bg-gray-50 p-3 space-y-2 group hover:shadow-md hover:border-blue-500 transition-all cursor-pointer block text-left">
